...
public abstract class ProceduralTree extends Tree {
	 ...
	 * @param diraxis The list index for the axis to make the section perpendicular to. 
	 * 0 indicates the x axis, 1 the y, 2 the z. The section will extend along the other two axies
	...
	private void foliagecluster(int x, int y, int z) {
		...
		for (double i : level_radius) {
			this.crossection(x, y, z, i, 1, Forester.LEAFMAT, Forester.LEAFDATA);
			y += 1;
		}
	}
	 ...
	 * <br>
	 * start and end are the beginning and ending coordinates of form [x,y,z].<br>
     * startsize and endsize are the beginning and ending radius.<br>
	 */
	...
	private void makeroots(double rootbases[][]) {
		...
		for (int[] coord : this.foliage_coords) {
			...
			double[] rootbase = rootbases[Forester.RANDOM.nextInt(rootbases.length)];
			...
			if (Forester.ROOTS == ForesterRoots.TO_STONE || Forester.ROOTS == ForesterRoots.HANGING) {
				...
				int[] searchindex = { 0 };
				if (Forester.ROOTS == ForesterRoots.TO_STONE) {
					searchindex = Forester.STOPSROOTS;
				} else if (Forester.ROOTS == ForesterRoots.HANGING) {
				...
				if (dist < offlength) {
					...
					if (Forester.ROOTS == ForesterRoots.HANGING) {
						...
						this.taperedcylinder(endcoord, bottomcord, rootmid, endsize, Forester.WOODMAT, Forester.WOODDATA);
					}

				}
				this.taperedcylinder(startcoord, endcoord, rootstartsize, rootmid, Forester.WOODMAT, Forester.WOODDATA);
			} else {
				this.taperedcylinder(startcoord, endcoord, rootstartsize, endsize, Forester.WOODMAT, Forester.WOODDATA);
			}
		}
	}
	...
	private void makebranches() {
		...
		for (int coord[] : this.foliage_coords) {
			...
			double endsize = 1.0;
			this.taperedcylinder(startcoord, coord, startsize, endsize, Forester.WOODMAT, Forester.WOODDATA);
		}
	}

	/**
	 * Initialise the internal values for the Tree object.
	 * Primarily, sets up the foliage cluster locations.
	 */
	@Override
	public void prepare() {
		int[] treeposition = this.pos;
		this.trunkradius = .618 * Math.sqrt(this.height * Forester.TRUNKTHICKNESS);
		int yend;
		if (this.trunkradius < 1) {
			this.trunkradius = 1;
		}
		if (Forester.BROKENTRUNK) {
		...
		this.branchdensity = Forester.BRANCHDENSITY / Forester.FOLIAGEDENSITY;

		int topy = treeposition[1] + (int) (this.trunkheight + 0.5D);

		int ystart = treeposition[1];
		int num_of_clusters_per_y = (int) (1.5 + Math.pow((Forester.FOLIAGEDENSITY * this.height / 19.), 2));
		...
		for (int y : Forester.range(yend, ystart, -1)) {
			for (int i = 0; i < num_of_clusters_per_y; i++) {
				...
				if (Forester.STOPSBRANCHES.length != 0) {
	...
	public void makefoliage() {
		...
		for (int[] cord : foliage_coords) {
			Forester.assign_value(cord[0], cord[1], cord[2], Forester.WOODMAT, Forester.WOODDATA, this.mcmap);
			if (Forester.LIGHTTREE == 1) {
				Forester.assign_value(cord[0], cord[1] + 1, cord[2], Forester.LIGHTMAT, Forester.LIGHTDATA, this.mcmap);
			} else if (Forester.LIGHTTREE == 2 || Forester.LIGHTTREE == 3 || Forester.LIGHTTREE == 4) { // jank
				Forester.assign_value(cord[0] + 1, cord[1], cord[2], Forester.LIGHTMAT, Forester.LIGHTDATA, this.mcmap);
				Forester.assign_value(cord[0] - 1, cord[1], cord[2], Forester.LIGHTMAT, Forester.LIGHTDATA, this.mcmap);
				if (Forester.LIGHTTREE == 4) {
					Forester.assign_value(cord[0], cord[1], cord[2] + 1, Forester.LIGHTMAT, Forester.LIGHTDATA, this.mcmap);
					Forester.assign_value(cord[0], cord[1], cord[2] - 1, Forester.LIGHTMAT, Forester.LIGHTDATA, this.mcmap);
				}
			}
		}
	...
	public void maketrunk() {
		...
		if (Forester.ROOTBUTTRESSES || Forester.SHAPE == ForesterShape.MANGROVE) {
			...
			for (i = 0; i < num_of_buttresses; i++) {
				...
				this.taperedcylinder(new int[] { thisx, starty, thisz }, new int[] { x, midy, z },
						thisbuttressradius, thisbuttressradius, Forester.WOODMAT, Forester.WOODDATA);
				rootbases[i][0] = thisx;
				rootbases[i][1] = thisz;
		...
		this.taperedcylinder(new int[] { x, starty, z }, new int[] { x, midy, z }, startrad, midrad, Forester.WOODMAT,
				Forester.WOODDATA);

		this.taperedcylinder(new int[] { x, midy, z }, new int[] { x, topy, z }, midrad, endrad, Forester.WOODMAT, Forester.WOODDATA);

		this.makebranches();

		if (Forester.ROOTS == ForesterRoots.YES || Forester.ROOTS == ForesterRoots.TO_STONE || Forester.ROOTS == ForesterRoots.HANGING) {
		...
		if (trunkradius > 2 && Forester.HOLLOWTRUNK) {
			...
			this.taperedcylinder(new int[] { x, midy, z }, new int[] { x, hollow_top_y, z }, mid_radius, top_radius,
					Forester.TRUNKFILLMAT, Forester.TRUNKFILLDATA);
		}
	}
}