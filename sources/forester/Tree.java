...
public abstract class Tree {
	...
	/*
	 * Tree settings, these are just the defaults, these are prefixed with tree_ to
	 * prevent conflicts with other sub-class instance fields
	 */
	/**
	 * Which shapes would you like the trees to be? these first three are best
	 * suited for small heights, from 5 - 10<br>
	 * <br>
	 ...
	 * PALM a trunk with a fan at the top, only gets taller and shorter, these last
	 * four are best suited for very large trees, heights greater than 8<br>
	 ...
	 * RAINFOREST many slender trees, most at the lower range of the height, with a
	 * few at the upper end.<br>
	 ...
	 * 1.0 the trunk will extend to the top of the tree, and there will be no
	 * crown<br>
	 * 2.0 the trunk will extend out the top of the foliage, making the tree
	 * appear<br>
	 ...
	 * removes about half of the top of the trunk, and any foliage and branches that
	 * would attach above it.<br>
	 * Only works if SHAPE is not a "stickly" sub-type This results in trees that
	 * are shorter than the height settings<br>
	 * <br>
	...
	/**
	 * How many branches should there be? General multiplier for the number of
	 * branches However,<br>
	 * it will not make more branches than foliage clusters so to guarantee a branch
	 * to every foliage cluster,<br>
	 * set it very high, like 10000 this also affects the number of roots, if they
	 * are enabled.<br>
	 * <br>
	 * Examples:<br>
	 * 1.0 is normal<br>
	 ...
	 * TOSTONE roots will be stopped by stone (default see STOPSROOTS below). There
	 * may be some penetration.<br>
	 * HANGING will hang downward in air. Good for "floating" type maps (I really
	 * miss "floating" terrain as a default option)<br>
	 ...
	 * These make the trunk not-round at the base, seen in tropical or old
	 * trees.<br>
	...
	/**
	 * Add lights in the middle of foliage clusters for those huge trees that get so
	 * dark underneath<br>
	...
	/* Tree materials */
	/** wood ID */
	public int tree_WOODMAT = 17;
	/** wood metadata */
	public int tree_WOODDATA = 0;
	/** leaf ID */
	public int tree_LEAFMAT = 18;
	/** leaf metadata */
	public int tree_LEAFDATA = 0;
	/** light ID */
	public int tree_LIGHTMAT = 50;
	/** light metadata */
	public int tree_LIGHTDATA = 0;
	/** trunk fill ID */
	public int tree_TRUNKFILLMAT = 0;
	/** trunk fill metadata */
	public int tree_TRUNKFILLDATA = 0;

	/** world max height */
	public int world_max_height = 127;

	/** what kind of blocks should stop roots */
	public int[] tree_STOPSROOTS = new int[] { 1, 7 };
	/**
	 * what kind of blocks should stop branches (seems to be broken on non-flat
	 * worlds)
	 */
	...
	/**
	 * Make sure the parameters are not out of range to prevent out of bounds stuff,
	 * should be called before anything below
	 */
	private void checkParameters() {
		if (tree_SHAPE == null)
			tree_SHAPE = TreeShape.ROUND;
		if (tree_EDGEHEIGHT < 1)
			tree_EDGEHEIGHT = 1;
		if (tree_TRUNKTHICKNESS < 0.0D)
			tree_TRUNKTHICKNESS = 0.0D;
		if (tree_TRUNKHEIGHT < 0.0D)
			tree_TRUNKHEIGHT = 0.0D;
		if (tree_ROOTS == null)
			tree_ROOTS = TreeRoots.NO;
		if (tree_FOLIAGEDENSITY < 0.0D)
			tree_FOLIAGEDENSITY = 0.0D;
		if (tree_BRANCHDENSITY < 0.0D)
			tree_BRANCHDENSITY = 0.0D;
		if (tree_LIGHTTREE < 0 || tree_LIGHTTREE > 4)
			tree_LIGHTTREE = 0;
	}
	...
	/**
	 * Generate the foliage and enter it in mcmap. Note, foliage will disintegrate
	 * if there is no log nearby
	 */
	public abstract void makefoliage();

	@Override
	public String toString() {
		return String.format("Tree{pos=(%d,%d,%d),height=%d,type=%s}", this.pos[0], this.pos[1], this.pos[2],
				this.height, this.getClass().getSimpleName());
	}
	...
	public static Tree getTree(MCWorldAccessor mcmap, TreeShape shape) {
		...
		switch (shape) {
		case NORMAL:
			tree = new NormalTree(mcmap);
			break;
		case BAMBOO:
			tree = new BambooTree(mcmap);
			break;
		case PALM:
			tree = new PalmTree(mcmap);
			break;
		case ROUND:
			tree = new RoundTree(mcmap);
			break;
		case CONE:
			tree = new ConeTree(mcmap);
			break;
		case RAINFOREST:
			tree = new RainforestTree(mcmap);
			break;
		case MANGROVE:
			tree = new MangroveTree(mcmap);
			break;
		default:
			return null;
		}
